...
abstract class List[+T] {
  ...
  def `::`[U >: T](head: U): List[U] = new ::(head, this)
...
// The Nil object defines an empty list.
// The Nil object inherits from type `List[Nothing]`.
...
case object Nil extends List[Nothing] {
  ...
  def tail: List[Nothing] =
    throw new UnsupportedOperationException("tial of empty list")
}
...
final case class `::1`[T](hd: T, tl: List[T]) extends List[T] {
...
// In fact, this pattern can be abbreviated by letting the parameters directly implement
//   the `head` and `tail` methods of the superclass List, as in the following equivalent
//   but shorter definition of the `::` class:
final case class ::[T](head: T, tail: List[T]) extends List[T] { def isEmpty = false }
----
