...
* A SACK option that specifies _n_ blocks has a length of 8n + 2 bytes, so the 40 bytes available to hold TCP options can specify a maximum of four blocks.
...
+
The receiver is missing the sequence number range [1449,7241], which corresponds to the single 1448-byte packet starting with sequence number 1449.
...
+
This SACK is a duplicate ACK for sequence number 1449 and suggests that the receiver now requires two full-size segments starting with sequence numbers 1449 and 5793.
// +
// The sender reacts immediately by initiating fast retransmit, but because of congestion control procedures, the sender sends only one retransmission, for segment 1449. With the arrival of two additional ACKs, the sender is permitted to send its second retransmission, for segment 5793.
