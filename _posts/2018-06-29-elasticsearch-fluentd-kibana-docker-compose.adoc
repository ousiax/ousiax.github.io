...
*ELK* is the arconym for three open source projects: {elasticsearch}[**E**lasticsearch], Logstash, and {kibana}[**K**ibana].
...
{fluentbit-io}[Fluent Bit] collects and process logs (records) from different {input-plugins}[*input*] sources and allows to {parsers}[*parse*] and {filters}[*filter*] these records before they hit the {buffering-and-storage}[*Storage*] interface. Once data is processed and it's in a safe state (either in memory or the file system), the records are routed through the proper {outputs}[*output*] destinations.
