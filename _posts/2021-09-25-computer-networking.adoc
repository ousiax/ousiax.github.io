...
|Internet layer
|{internet_protocol}[IP](IPv4 IPv6) {internet_control_message_protocol}[ICMP](v6) ECN {internet_group_management_protocol}[IGMP] {ipsec}[IPsec]

|Link layer
|{address_resolution_protocol}[ARP] NDP {open_shortest_path_first}[OSPF] {tunneling_protocol}[Tunnels](L2TP) PPP {medium_access_control}[MAC](Ethernet Wi-Fi DSL ISDN FDDI)
|====
