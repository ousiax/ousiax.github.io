...
. Open `https://echo.local.io` on Windows Chrome Browser
+
image::/assets/hsts/echo-local-io-hsts.png[75%,75%,title="Open `https://echo.local.io` on Windows Chrome Browser"]

. then open `chrome://net-internals/#hsts` and `Query HSTS/PKP domain` with `echo.local.io`.
+
...
+
[source,console,linenums,highlight=10..14]
----
Found:
static_sts_domain:
static_upgrade_mode: UNKNOWN
