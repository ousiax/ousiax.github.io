...
* Such a type is said to _implement the interface_.

* The value of an uninitialized variable of interface type is `nil`.

An interface type is specified by a list of _interface elements_.

* An interface element is either a _method_ or a _type element_, where a type element is a union of one or more type terms.

* A _type term_ is either a single type or a single _underlying type_. 
