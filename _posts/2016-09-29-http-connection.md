...
套接字 API 最初是为 Unix 操作系统开发的，但现在计划所有的操作系统和语言中都有其变体存在。
...
HTTP 紧挨着 TCP，位于其上层，所以 HTTP 食物的性能很大程度取决于底层 TCP 通道的性能。
...
- 并行连接
    - 并行连接可能会提高页面的加载速度
        ...
        包含嵌入对象的组合如果能（通过并行连接）可否单条连接的空载时间和带宽限制，加载速度也会有所提高。时延可以重叠起来，而如果单条连接没有充分利用客户端的因特网带宽，可以将未用带宽分配来装载其他对象。
    - 并行连接不一定更快
