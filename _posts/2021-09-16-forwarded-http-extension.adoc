= Proxy Server and Forwarded HTTP Extension
:page-layout: post
:page-categories: []
:page-tags: []
...
<2> XFF: Identifies the originating IP addresses of a client connecting to a web server through an HTTP proxy or a load balancer.
<3> XFH: Identifies the original host requested that a client used to connect to your proxy or load balancer.
<4> _: Identifies the port that a client used to connect to your proxy or load balancer.
<5> XFP: Identifies the protocol (HTTP or HTTPS) that a client used to connect to your proxy or load balancer.
