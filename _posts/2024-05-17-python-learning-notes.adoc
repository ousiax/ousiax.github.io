...
```py
# animal.py : a module is just a file of any Python code.
class Animal:
...
# main.py
from animals import cat, dog  # `from animals` looks for a directory named `animals`, starting under the current directory
cat.wow()  # meow!
dog.wow()  # bark!
```
