...
数字824也就是数字-176在有符号集合中的表示。我们再看下10的补数表示负数的被减数253减去减数176的减法的运算：
...
到此为止，我们知道在十进制数的减法中，通过9的补数可以避免减法中烦琐的借位运算，通过10的补数表示负数，可以将减法运算转换为加法运算。这也是如古老的机械计算器(如基于齿轮的帕斯卡计算器)以及现代电子计算机的有符号数的表示与应用原理。
...
结果是510，我们知道以数字5开头的表示负数，也就是说两个正数的和是个负数？对510求10的补数可以得到原来的数字-490。是不是很神奇？对的，这种情况我们称之为溢出，严格一点叫做上溢(overflow)，也就是说两个3位数的运算的结果超出了3位有符号数能够表示的上界。
...
结果是1024，最高位1舍弃，最终的到数字24，我们知道24是一个正数，也就是两个负数相加的结果是个正数？对的，这是另外一种溢出，因为是超出了3位有符号数的下界，我们称之为下溢(underflow)。
...
上述的二进制数的减法中用到了1的补数的知识，同十进制数，我们也二进制数的2的补数(two's complement)，比如对于数字176的二进制数1111-1101的2的补数可以用1-0000-0000减去1111-1101计算。当然，为了避免借位减法，我们可以计算数字1111-1101的1的补数再加1即可。
...
对于二进制数的1的补数，只需要将1转换为0，0转换为1即可，所以二进制的1的补数又叫做反码(inverse)。二进制数的2的补数只需要用1的补数加1即可，2的补数又叫做补码。而原始的二进制数也称为原码(true code)。显然，反码的反码是原码，那么补码的补码是什么呢？还是原码。比如对于原码1111-1101的补码0000-0011求补码运算：
...
二进制数的反码和补码在逻辑电路中实现起来非常简单，对于8位二进制数的反码运算，只需要用8位反相器(inverter)。
...
结果是0100-1101，也就是十进制数的77，再加上负号，即数字-77。
...
我们发现，除了数字1000-0000外，在数字0000-0000两侧对称的数字互为2的补数。我们可以用1开头的8位二进制数表示负数，以0开头的8位二进制数表示整数：
...
我们看到结果1110-1001的最高位是1，说明这是一个负数。因为被减数小于减数，这也是我们期望的结果。
