...
现在，我们来看一下有符号二进制数的溢出(overflow)问题。
...
我们看到两个正数124和16的加法的和是1000-1100，最高位是1表示是负数？对的，这就是我们所说的上溢。
...
舍弃最高位的1，运算的结果为0000-0001。因为最高位是0，所以是个正数。两个负数相加的结果是个负数？对的，这是就是说8位二进制数不能够表示小于-128的负数，产生了下溢。
