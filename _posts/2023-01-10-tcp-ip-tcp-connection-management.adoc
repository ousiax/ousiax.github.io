...
[TIP]
====
Both the `close()` and `shutdown()` system calls in TCP networking will trigger the sending of a FIN (Finish) TCP segment to the peer end. This part of the TCP process is known as "closing a connection" or "TCP teardown", signaling that the sender will send no further data.

However, even though they both end up sending a FIN packet, there's a difference between the two:

1. The `close()` call deallocates the socket right away. So any further operation on this socket is not possible. If there's any unread data in the socket's receive buffer, this data is discarded. Also, `close()` will immediately return whether the TCP FIN process completes or not.

2. The `shutdown()` call, on the other hand, can be configured to shut down just sending, just receiving, or both sending and receiving on the socket (using `SHUT_RD`, `SHUT_WR`, or `SHUT_RDWR`). Use `shutdown()` when you want to inform the peer that you are done sending (or receiving), but still want to continue receiving (or sending) data that the peer had already sent.
+
--
* `shutdown(socket, SHUT_RD)` (or 0) - Further receives will be disallowed (this is a local operation and doesn't notify the peer), no FIN sent.
+
The `shutdown()` function with the `SHUT_RD` argument disables further receive operations on the socket. This means that the application will not be able to read data from the kernel buffer associated with this socket.
+
However, the peer end can still send data. The system (i.e., the TCP stack in the kernel) will acknowledge any received data to maintain the correctness of the TCP state, but the data will not be accessible via the shut-down socket. This data will essentially be discarded.
+
So, calling `shutdown(socket, SHUT_RD)` doesn't inform the peer that it should stop sending data. It just tells your local system to stop delivering received data to the application.

* `shutdown(socket, SHUT_WR)` (or 1) - Further sends will be disallowed and sends a FIN packet to the peer to initiate connection closure.

* `shutdown(socket, SHUT_RDWR)` (or 2) - Further sends and receives will be disallowed and sends a FIN packet to the peer to initiate connection closure.

* Remember, `SHUT_WR` and `SHUT_RDWR` are the only modes that initiate termination of the connection, which includes sending `FIN` to the peer.
--
====
...
1. **Encryption**: The most crucial defense is that communication between the client and server is encrypted. Even if an attacker is successful in intercepting the packets, they would not be able to understand the encrypted contents without the proper decryption keys.

2. **Authentication**: TLS/SSL mandates certificate-based authentication. This means that the entities involved in the communication must present digital certificates to validate their identities. This authentication helps to prevent "man-in-the-middle" attacks (where an attacker intercepts and may alter communication between two parties without their knowledge).

3. **Integrity Checks**: TLS/SSL protocols include a Message Authentication Code (MAC), which provides a way to check data integrity. Each message packet contains a checksum (MAC), which is calculated using data contained in the packet along with a secret key. The recipient also calculates its own MAC for the packet and verifies it against the one sent with the packet. If the MACs do not match, it means the data was tampered with during transmission, defending against any injection attacks such as forged overlapping packets.

4. **Sequence Numbers**: TLS/SSL also specifies that every record (data unit) that is transmitted through a TLS/SSL connection includes a sequence number. The receiver verifies this sequence number before accepting the record, which also defends against replay or injection attacks.

5. **Key Exchange Mechanisms**: Secure key exchange mechanisms, like the Diffie-Hellman algorithm, are used to securely exchange keys, thwarting attempts to predict or brute-force the encryption keys.
...
tcp        0      0 127.0.0.1:54043         0.0.0.0:*               LISTEN
tcp        0      0 0.0.0.0:22              0.0.0.0:*               LISTEN
tcp        0      0 0.0.0.0:4000            0.0.0.0:*               LISTEN
tcp        0      0 127.0.0.1:44363         0.0.0.0:*               LISTEN
tcp        0      0 0.0.0.0:35729           0.0.0.0:*               LISTEN
tcp        0      0 127.0.0.1:54043         127.0.0.1:42982         TIME_WAIT
tcp        0      0 192.168.91.128:35729    192.168.91.1:58032      ESTABLISHED
tcp        0    172 192.168.91.128:22       192.168.91.1:62703      ESTABLISHED
tcp        0      0 192.168.91.128:35729    192.168.91.1:58042      ESTABLISHED
tcp        0      0 192.168.91.128:35729    192.168.91.1:58034      ESTABLISHED
tcp        0      0 192.168.91.128:35729    192.168.91.1:58036      ESTABLISHED
tcp        0      0 127.0.0.1:54043         127.0.0.1:43022         FIN_WAIT2
tcp        0      0 192.168.91.128:35729    192.168.91.1:58043      ESTABLISHED
tcp        0      0 127.0.0.1:54043         127.0.0.1:43038         TIME_WAIT
tcp        0      0 192.168.91.128:22       192.168.91.1:62518      ESTABLISHED
tcp        0      0 127.0.0.1:54043         127.0.0.1:43026         TIME_WAIT
tcp        0      0 127.0.0.1:54043         127.0.0.1:43006         FIN_WAIT2
tcp        1      0 127.0.0.1:43022         127.0.0.1:54043         CLOSE_WAIT
tcp        0      0 127.0.0.1:54043         127.0.0.1:42998         FIN_WAIT2
tcp        0      0 192.168.91.128:35729    192.168.91.1:58044      ESTABLISHED
tcp        0      0 192.168.91.128:22       192.168.91.1:62227      ESTABLISHED
tcp        0      0 192.168.91.128:35729    192.168.91.1:58041      ESTABLISHED
tcp        1      0 127.0.0.1:42998         127.0.0.1:54043         CLOSE_WAIT
tcp        0      0 192.168.91.128:22       192.168.91.1:62684      ESTABLISHED
tcp        3      0 127.0.0.1:43006         127.0.0.1:54043         CLOSE_WAIT
x@node-0:~$ ss -nat4
...
LISTEN      0       5             127.0.0.1:54043        0.0.0.0:*
LISTEN      0       128             0.0.0.0:22           0.0.0.0:*
LISTEN      0       4096            0.0.0.0:4000         0.0.0.0:*
LISTEN      0       4096          127.0.0.1:44363        0.0.0.0:*
LISTEN      0       100             0.0.0.0:35729        0.0.0.0:*
TIME-WAIT   0       0             127.0.0.1:54043      127.0.0.1:42982
ESTAB       0       0        192.168.91.128:35729   192.168.91.1:58032
ESTAB       0       0        192.168.91.128:22      192.168.91.1:62703
ESTAB       0       0        192.168.91.128:35729   192.168.91.1:58042
ESTAB       0       0        192.168.91.128:35729   192.168.91.1:58034
ESTAB       0       0        192.168.91.128:35729   192.168.91.1:58036
FIN-WAIT-2  0       0             127.0.0.1:54043      127.0.0.1:43022
ESTAB       0       0        192.168.91.128:35729   192.168.91.1:58043
TIME-WAIT   0       0             127.0.0.1:54043      127.0.0.1:43038
ESTAB       0       0        192.168.91.128:22      192.168.91.1:62518
TIME-WAIT   0       0             127.0.0.1:54043      127.0.0.1:43026
FIN-WAIT-2  0       0             127.0.0.1:54043      127.0.0.1:43006
CLOSE-WAIT  1       0             127.0.0.1:43022      127.0.0.1:54043
FIN-WAIT-2  0       0             127.0.0.1:54043      127.0.0.1:42998
ESTAB       0       0        192.168.91.128:35729   192.168.91.1:58044
ESTAB       0       0        192.168.91.128:22      192.168.91.1:62227
ESTAB       0       0        192.168.91.128:35729   192.168.91.1:58041
CLOSE-WAIT  1       0             127.0.0.1:42998      127.0.0.1:54043
ESTAB       0       0        192.168.91.128:22      192.168.91.1:62684
CLOSE-WAIT  3       0             127.0.0.1:43006      127.0.0.1:54043
----
+
[source,console]
----
...
1. **End Connection Reliably**: TIME_WAIT assures that the ACK (acknowledgement) sent by the system that closes the connection is received by the system that first requested the shutdown. This allows both sides of the connection to cleanly and synchronously finish all data transmission before going away. If the final ACK were to get lost in the network, the side which sent the (second) last packet (called FIN), would resend it, and the TIME_WAIT side could still ACK it (and not confuse the FIN with new data for a new, future connection).

2. **Avoid Data Overlap**: By keeping the connection around in the TIME_WAIT state, the system ensures that all segments of data from this connection are drained out of the network before the same IP:Port to IP:Port mapping is reused. If this didn't happen and such a mapping was reused quickly, new data could theoretically be mixed up with old, late-arriving data from the previous connection, which would confuse the receivers.
...
[TIP]
====
- `SO_REUSEADDR` is a socket option that allows a socket to bind to a port that is currently in use, typically, in the `TIME_WAIT` state. Its most common use case is to allow server applications to restart and immediately re-bind to the same port. However, using `SO_REUSEADDR` can pose a risk of data confusion, as it might receive delayed packets from the prior connection, and as such should be used with caution.

- `SO_REUSEPORT` is another socket option that allows multiple sockets on the same host to bind to an exactly identical address-port combination. It is most often used to distribute incoming connections over multiple threads or processes for load balancing. The kernel ensures that all packets of a specific network connection are sent to the same socket. This reduces the likelihood of packet confusion.

- When a server binds to a wildcard address (like `0.0.0.0` or `+++*+++`), it's telling the system to listen for and accept incoming connections on a specified port, on any of the host's network addresses. `SO_REUSEPORT` allows multiple sockets (across different processes or threads) on the same host to bind to the same `+++*+++:port`, each of which can separately accept incoming connections.

- The kernel directs incoming connections to `SO_REUSEPORT` sockets using either a round-robin or a hash-based distribution algorithm, depending on the specific operating system. This makes certain that all packets from a single connection are directed to the same socket, even if there are multiple sockets bound to the same address-port combination.

Both `SO_REUSEADDR` and `SO_REUSEPORT` have their own potential issues and have to be used with proper understanding of their implications for data integrity and security.
====
